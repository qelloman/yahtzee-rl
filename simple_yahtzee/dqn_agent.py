...
def state_to_tensor(state):
    ...
    tensor_out = torch.tensor([num_to_roll] + category_filled + dice_count, dtype=torch.float)
...
class DQN(nn.Module):
    def __init__(self, num_dice=4, num_eyes=6, num_category=6):
        super(DQN, self).__init__()
        # num_dice_face => score_category => binary
        # num_dice => hexary
        self.fc1 = nn.Linear(1 + num_eyes + num_category, 64)
        self.bn1 = nn.BatchNorm1d(64)
        self.fc2 = nn.Linear(64, 128)
        self.bn2 = nn.BatchNorm1d(128)
        self.fc3 = nn.Linear(128, pow(2, num_dice)+num_category)
...
class DQNAgent(object):
    def __init__(self, num_dice=4, num_eyes=6, num_category=6, learning_rate=LEARNING_RATE):
        self.policy_net = DQN(num_dice=num_dice, num_eyes=num_eyes, num_category=num_category)
        self.target_net = DQN(num_dice=num_dice, num_eyes=num_eyes, num_category=num_category)
    ...
    def optimize_model(self):
        ...
        writer.add_scalar("train/loss", loss, epoch_cnt)
...
def learn(num_episodes=500, learning_rate=LEARNING_RATE, save_file='dqn_model.dat'):
    ...
    num_dice = 4
    num_eyes = 6
    env = SimpleYahtzeeEnv(num_dice, num_eyes, num_eyes, mode='comp')
    agent = DQNAgent(num_dice, num_eyes, num_eyes, learning_rate=LEARNING_RATE)
...
def play(num_episode):
    ...
    env = SimpleYahtzeeEnv(num_dice, num_eyes, num_eyes, mode='comp')
    agent = DQNAgent(num_dice, num_eyes, num_eyes, learning_rate=LEARNING_RATE)
    model_dir = input("Type your model directory")
    if model_dir is not '':
        model_path = './dqn/' + model_dir + '/dqn_model.dat'
        if os.path.exists(model_path):
            agent.load_policy_net(model_path)
    total_scores = []
