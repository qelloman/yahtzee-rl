...
class A2C(nn.Module):
    def __init__(self, num_dice=4, num_eyes=6, num_category=6):
        ...
        self.action_fc = nn.Linear(128, pow(2, num_dice) + num_category)
...
class A2CAgent(object):
    def __init__(self, num_dice=4, num_eyes=6, num_category=6, learning_rate=LEARNING_RATE):
        self.a2c_net = A2C(num_dice=num_dice, num_eyes=num_eyes, num_category=num_category)
...
def learn(num_episodes=500, learning_rate=LEARNING_RATE, save_file='a2c_model.dat'):
    ...
    agent.load_a2c_net('./a2c/20220312-131928/a2c_model.dat')
...
def play(num_episode):
    num_dice = 4
    num_eyes = 6
    env = SimpleYahtzeeEnv(num_dice, num_eyes, num_eyes, 'comp')
    agent = A2CAgent(num_dice, num_eyes, num_eyes, learning_rate=LEARNING_RATE)
