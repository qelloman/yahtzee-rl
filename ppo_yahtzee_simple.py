...
def parse_args():
    ...
    parser.add_argument("--gym-id", type=str, default="YahtzeeSimple-v0",
...
def eval(agent, env, eval_num, device, log_dir):
    ...
    with open(f'{log_dir}/eval_log.txt', 'a') as f:
        ...
        all_episode_rewards = []
        total_num_action = 0
        total_num_invalid_action = 0
        total_num_hold_action = 0
        for eval_idx in range(eval_num):
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    total_num_action += 1
                    if action.item() < 31:
                        total_num_hold_action += 1
                    if not info['action_mask'][action]:
                        total_num_invalid_action += 1
        ...
        f.write("=" * 30 + "\n\n")
        return np.mean(all_episode_rewards), float(total_num_invalid_action / total_num_action), float(total_num_hold_action / total_num_action)


class Agent(nn.Module):
    def __init__(self, envs):
        super(Agent, self).__init__()
        total_size = np.prod(envs.single_observation_space.shape)
        self.critic = nn.Sequential(
            layer_init(nn.Linear(total_size, 64)),
            nn.Tanh(),
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        writer.add_scalar("losses/wrong_action", wrong_action, global_step)
        print("SPS:", int(global_step / (time.time() - start_time)))
        writer.add_scalar("charts/SPS", int(global_step / (time.time() - start_time)), global_step)

        if update % args.eval_freq == 0:
            eval_reward, invalid_ratio, hold_ratio = eval(agent, eval_env, args.eval_num, device, log_dir=f"runs/{run_name}")
            writer.add_scalar("eval/return", eval_reward, global_step)
            writer.add_scalar("eval/invalid_ratio", invalid_ratio, global_step)
            writer.add_scalar("eval/hold_ratio", hold_ratio, global_step)
