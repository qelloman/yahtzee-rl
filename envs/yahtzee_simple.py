...
class YahtzeeSimpleEnv(gym.Env):
    def __init__(self):
        super(YahtzeeSimpleEnv, self).__init__()
        ...
        num_obs = self.num_dice * 6 + self.num_categories * 2 + self.rolls_left
        self.observation_space = spaces.Box(low=0, high=1, shape=(num_obs,), dtype=np.float32)
    ...
    def _get_observation(self):
        ...
        scorecard_arr = self.scorecard.astype(float) / self.max_score
        cateogry_filled_arr = self.category_filled.astype(float)
    ...
    def step(self, action):
        ...
        if np.all(self.category_filled):
            done = True
